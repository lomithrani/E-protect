  ...
  ***************************************************************************/
  onConnect: function(session, socket) {
        ...
        socket.on('disconnect', function(data) {
          ...
          if(session.sigfox){//disconnect
            var sigfox = session.sigfox;
            sigfox.online = false;
            sigfox.save(function(err, user) {
                 if (err) return next(err);

                     session.sigfox = sigfox;
                   // Inform other sockets (e.g. connected sockets that are subscribed) that this user is now logged in
          Sigfox.publishUpdate(sigfox.id, {
            online: false,
            id: sigfox.id,
            name: user.name,
       ...
       socket.on('connect', function(data) {
        ...
        console.log('looking for sigfox:' + data.sigfox);
        Sigfox.findOneBySigfoxId(data.sigfox, function foundSigfox(err, sigfox) {
        ...
        sigfox.save(function(err, user) {
          ...
          session.sigfox = sigfox;
          // Inform other sockets (e.g. connected sockets that are subscribed) that this user is now logged in
          console.log('sigfox publish online');
          Sigfox.publishUpdate(sigfox.id, {
            online: true,
            id: sigfox.id,
            name: user.name,
        ...
        socket.on('update', function(data) {
          if(session.sigfox){
            ...
            Device.findOne({owner: session.sigfox.id, idDevice: data.id},function foundDevice(err, device) {
             
              if(!device){//create device
              var params = {};
              params.type=data.ty;
              params.idDevice=data.id;
              params.state=data.st;
              params.owner=session.sigfox;
              Device.create(params).exec(function createCB(err,device){
              ...
              }else if(device.state != data.st){//update device 
                console.log('device found , update');
                device.state = data.st;

                device.save(function(err, user) {
                  if (err) return next(err);

                  Device.publishUpdate(device.id, {
                      state: device.state,
                      id: device.id,
                      type: device.type,
                      action: 'has changed type'
                  });
