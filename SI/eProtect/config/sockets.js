...
console.log('session'+ session);
       ...
       socket.on('connect', function(data) {
        ...
        console.log('looking for :' + data.sigfox);
        ...
        socket.on('update', function(data) {
          if(session.sigfox){
            Device.findOne({owner: session.sigfox.id, id: data.id},function foundDevice(err, device) {
              ...
              Device.create(params).exec(function createCB(err,device){
                  console.log('tentative de creation device');
                  if(device)
                  console.log('device created');
                if(err)
                  console.log(err)
                });
              }else if(device.state != data.state){//update device 
